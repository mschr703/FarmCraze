...
class EventManager:
    """Verwaltet zufällige Spiel-Events wie UFOs oder Stürme."""
    ...
    def update(self, dt):
        """Aktualisiert den Zustand der Events."""
        if self.is_active:
            self.event_timer -= dt
        ...
        # Prüfen, ob ein neues Event gestartet werden soll
    ...
    def _check_for_new_event(self):
        """Prüft zufällig, ob ein Event ausgelöst wird."""
    ...
    def _prepare_event(self, event_type):
        """Startet den Countdown für ein bevorstehendes Event."""
        self.pre_event_active = True
        self.active_event_type = event_type
        self.pre_event_timer = settings.PRE_EVENT_DURATION

    def _start_event(self):
        """Aktiviert das Event und seine Effekte."""
        self.pre_event_active = False
        self.is_active = True
    ...
    def _end_event(self):
        """Beendet das Event und setzt die Effekte zurück."""
        if self.active_event_type == "ufo":
            self._end_ufo_event()
        elif self.active_event_type == "storm":
    ...
    def _start_ufo_event(self):
        """Startet die UFO-Logik."""
    ...
    def _teleport_sheep(self, sheep):
        """Teleportiert ein einzelnes Schaf an eine zufällige Position."""
    ...
    def _end_ufo_event(self):
        """Beendet die UFO-Logik."""
    ...
    def _start_storm_event(self):
        """Startet die Sturm-Logik."""
    ...
    def _end_storm_event(self):
        """Beendet die Sturm-Logik."""
        if self.assets.sounds["regen"]:
    ...
    def _run_active_event_logic(self):
        """Logik, die während eines aktiven Events ausgeführt wird."""
    ...
    def draw(self, surface):
        """Zeichnet die visuellen Effekte des Events."""
    ...
    def _draw_pre_event_text(self, surface):
        """Zeichnet den "Ein Event braut sich zusammen..." Text."""
    ...
    def _draw_active_event_ui(self, surface):
        """Zeichnet Titel und Beschreibung des aktiven Events."""
        if self.active_event_type == "ufo":
    ...
    def _init_raindrops(self):
        """Initialisiert die Regentropfen für den Sturm."""
        self.raindrops = [{
    ...
    def _update_raindrops(self):
        """Bewegt die Regentropfen."""
        for drop in self.raindrops:
            drop["y"] += drop["speed"] * 5 # Geschwindigkeit anpassen
            if drop["y"] > settings.VIRTUAL_HEIGHT:
                drop["y"] = 0
                drop["x"] = random.randint(0, settings.VIRTUAL_WIDTH)

    def _draw_storm_overlay(self, surface):
        """Zeichnet den Regen und die Wolken."""
        # Regen
        for drop in self.raindrops:
